				...
				<div class="col-sm-9">
					<div class="blog-post-area">
						<h2 class="title text-center">Latest From our Blog</h2>
							<div class="single-blog-post">
								...
								<div class="post-meta">
									<ul>
										<li><i class="fa fa-user"></i>{{$getBlogListComment->userBlog->name}}</li>
									...
									<span>
										<!-- cách của mình -->
									@if($getBlogListComment->rates->count() != 0)
										@php $rating = $getBlogListComment->rates->avg('vote'); @endphp
								...
								<div class="pager-area">
									<ul class="pager pull-right">
										@if($preBlog)
											<li>
												<a href="{{url('blog-details/'.$preBlog)}}">Pre</a>
											</li>
										@endif
										@if($nextBlog)
											<li>
												<a href="{{url('blog-details/'.$nextBlog)}}">Next</a>
											</li>
										@endif
									</ul>
								</div>
					...
					<div class="rating-area">
						...
						<ul class="ratings rate">
							...
							<li class="color">({{$getBlogListComment->rates->count()}} votes)</li>
						</ul>
						@endif
						<ul class="tag">
							<li>TAG:</li>
					...
					<div class="response-area">
						<h2>{{$getBlogListComment->countComment}} RESPONSES</h2>
					@if($getBlogListComment->countComment >= 1)
						<ul class="media-list">
						@foreach($getBlogListComment->comments as $comment)
							@if($comment->id_comment == 0)
								<li class="media">
									<a class="pull-left" href="">
										<img class="media-object" src="{{ asset('admin/assets/images/users/'.$comment->userComment->avatar) }}" alt="" style="width: 140px; height: 102px;">
									</a>
									<div class="media-body">
										<ul class="sinlge-post-meta">
											<li><i class="fa fa-user"></i>{{$comment->userComment->name}}</li>
										...
										<p>{{$comment->comment}}.</p>
										<a id="{{$comment->id}}" class="btn btn-primary sub-comment" href="#comment" href="#comment"><i class="fa fa-reply"></i>Reply</a>
									</div>
								</li>
								@foreach($comment->subComments as $subComment)
								<li class="media second-media">
									<a class="pull-left" href="">
									<img class="media-object" src="{{ asset('admin/assets/images/users/'.$subComment->userComment->avatar) }}" alt=""  style="width: 140px; height: 102px;">
									</a>
									<div class="media-body">
										<ul class="sinlge-post-meta">
											<li><i class="fa fa-user"></i>{{$subComment->userComment->name}}</li>
											<li><i class="fa fa-clock-o"></i>{{$subComment->created_at->toTimeString()}}</li>
											<li><i class="fa fa-calendar"></i>{{$subComment->created_at->toDateString()}}</li>
										</ul>
										<p>{{$subComment->comment}}.</p>
										<a id="{{$comment->id}}" class="btn btn-primary sub-comment" href="#comment"><i class="fa fa-reply"></i>Reply</a>
									</div>
								</li>
								@endforeach
							@endif
							...
							<form action="" method="post" class="comment">
								...
								<div class="col-sm-8">
									...
									<div class="text-area">
										...
										<span>*</span>
										<input value="0" class="id_comment" type="hidden" name="id_comment" >
...
<script>
	$(document).ready(function(){
		// Kiểm tra bắt lỗi đăng nhập hay chưa để kích hoạt form
		$("form.comment").submit(function(){
			...
			if(checkLog == true) {
				if($(this).find('textarea').val() != false) {
					$('div.alert').removeClass('alert-danger');
					$('div.alert').find('li').html("");
					return true;
				} else {
					$('div.'+getClass).addClass('alert-danger');
					$('div.'+getClass).find('li').html("Bình luận không được để trống, xin hãy thử lại.");
				}
			} else {
				$('div.'+getClass).addClass('alert-danger');
	...
	$(document).ready(function(){
	    ...
	    $('.ratings_stars').click(function(e){
	        ...
	        var getUserId = <?= Auth::check() ? Auth::user()->id : 'false' ?>;
